...
export default function WorksPageSection() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      setLoading(true)
      try {
        const response = await fetch(`/api/youtube?playlist=${encodeURIComponent(selectedPlaylist)}&limit=50`)
      ...
      <section className="relative min-h-screen py-32 px-4 bg-dark-bg overflow-hidden">
        ...
        <div className="relative z-20 max-w-7xl mx-auto">
          {loading ? (
            ...
            <>
              <motion.div
                className="mb-4 text-white/70 text-sm text-center"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
